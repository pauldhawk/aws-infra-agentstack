...
// We assume that the n8n and Qdrant credentials already exist in Secrets Manager
// with the configured names.  Pulumi will not attempt to create them.  To
// reference these secrets in IAM policies, we construct their ARNs manually
// using the region and account ID.  The Aurora master password secret is
// created by this stack if it doesn't already exist.

// Fetch the AWS account ID for constructing ARNs
const callerIdentity = aws.getCallerIdentity();

const accountId = callerIdentity.then(id => id.accountId);

// Helper function to build a secret ARN from a name.  We append a trailing
// wildcard (*) to match the unique suffix generated by AWS when creating
// secrets via the console (e.g. `name-randomstring`).
function secretArn(name: string): pulumi.Output<string> {
    return pulumi.all([region, accountId]).apply(([r, account]) =>
        `arn:aws:secretsmanager:${r}:${account}:secret:${name}*`
    );
}

const n8nSecretArn = secretArn(n8nSecretName);
const qdrantSecretArn = secretArn(qdrantSecretName);
...
const dbCluster = new aws.rds.Cluster("aurora-cluster", {
    ...
    // engineVersion is intentionally omitted to allow AWS to select a supported
    // default version for Aurora PostgreSQL.  Setting an invalid version (e.g.
    // 13.6) will cause the API to reject the request.
    databaseName: "appdb",
...
const dbInstance = new aws.rds.ClusterInstance("aurora-instance", {
    ...
    // Explicitly specify engine; omit engineVersion so AWS selects a valid
    // version compatible with the cluster.
    engine: "aurora-postgresql",
...
const ec2RolePolicy = new aws.iam.RolePolicy("ec2-role-policy", {
    role: ec2Role.id,
    policy: pulumi.all([backupBucket.arn, n8nSecretArn, qdrantSecretArn, auroraPasswordSecret.arn]).apply(
...
const userData = pulumi.all([dbCluster.endpoint, auroraMasterUsername, backupBucket.bucket, n8nSecretName, qdrantSecretName]).apply(
    ([dbEndpoint, dbUser, backupBucketName, n8nSecretNameParam, qdrantSecretNameParam]) => {
        return `#!/bin/bash
set -eu
...
n8n_creds=$(aws secretsmanager get-secret-value --secret-id ${n8nSecretNameParam} --query SecretString --output text || echo "{{}}")
qdrant_creds=$(aws secretsmanager get-secret-value --secret-id ${qdrantSecretNameParam} --query SecretString --output text || echo "{{}}")
